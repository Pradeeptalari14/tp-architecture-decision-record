...
> Standardized design log templates recording framework selections and vector index schema versions.
...
## 🎛️ How It Works
Open the **[Interactive Studio](https://pradeeptalari14.github.io/portfolio/tools/architecture-decision-record/)** in your portfolio dashboard to customize options, select webhook notification channels (Slack/Teams), toggle chat bot connections (WhatsApp Business/Telegram Bot), and download compiled deployment scripts dynamically.
...
```bash
git clone https://github.com/Pradeeptalari14/tp-architecture-decision-record.git
cd tp-architecture-decision-record
```
...
```bash
kubectl port-forward deployment/tp-architecture-decision-record 8080:8080 || echo "Port forwarding enabled on localhost:8080"
```
...
```text
tp-architecture-decision-record/
...
└── primary_config/
    └── adr_decision.md     # Main configuration file
```
...
---
*Generated by [Architecture Decision Record (ADR) Studio](https://pradeeptalari14.github.io/portfolio/tools/architecture-decision-record/) — [Talari Pradeep SRE Portfolio](https://pradeeptalari14.github.io/portfolio)*
